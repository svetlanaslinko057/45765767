...
export default function ClientTransparency() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [transp, bill] = await Promise.all([
        axios.get(`${API}/client/transparency`,    { withCredentials: true }),
        axios.get(`${API}/client/billing/summary`, { withCredentials: true }),
      ]);
      setData(transp.data);
      setBill(bill.data);
    } catch (e) {
  ...
  const onContinue = async () => {
    setBusy(true);
    try {
      const r = await axios.post(`${API}/client/billing/continue`, {}, { withCredentials: true });
  ...
  const onAutoChargeToggle = async () => {
    ...
    try {
      await axios.post(`${API}/client/billing/auto-charge`, { enabled: next }, { withCredentials: true });
